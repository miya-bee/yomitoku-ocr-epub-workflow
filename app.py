...
class UnifiedYomitokuApp:
    ...
    def run_split2_preview(self):
        ...
        try:
            ...
            self._split2_update_preview(plan, summary_text=f"テストOK: 分割数 {len(plan)}（未出力）")
            self.lbl_split2_status.config(text="テスト完了", foreground="gray")

        except Exception as e:
            self.lbl_split2_status.config(text="テスト失敗", foreground="gray")
    ...
    def run_split2(self):
        ...
        try:
            ...
            if self._split2_preview_ctx:
                # 入力ファイルが変わっていないか確認
                if self._split2_preview_ctx["src_path"] == src_path:
                    # さらに内容長が一致するか（ファイル書き換わり対策）
                    if len(self._split2_preview_ctx["full_content"]) == total_length:
                        plan = self._split2_preview_ctx["plan"]
            ...
            if check_total_chars != total_length:
                diff = total_length - check_total_chars
                self.safe_showerror("重大なエラー", f"分割スライス合計が一致しません。\n元: {total_length}\n合計: {check_total_chars}\n差: {diff}")
            ...
            if reconstructed == full_content:
                ...
                self.safe_showinfo("完了", f"分割が完了しました（検証OK）\n\n出力フォルダ:\n{output_dir}\n\n結合順ファイル:\n{order_path}")

            else:
                mismatch_at = None
                min_len = min(len(reconstructed), len(full_content))
                for j in range(min_len):
                    if reconstructed[j] != full_content[j]:
                        mismatch_at = j
                        break
                if mismatch_at is None and len(reconstructed) != len(full_content):
                    mismatch_at = min_len

                around = ""
                if mismatch_at is not None:
                    a0 = max(0, mismatch_at - 40)
                    a1 = min(len(full_content), mismatch_at + 40)
                    around = (
                        f"\n\n不一致位置: {mismatch_at}\n"
                        f"元(周辺): {repr(full_content[a0:a1])}\n"
                        f"再(周辺): {repr(reconstructed[a0:a1])}"
                    )

                self.safe_showerror("検証NG", f"再構成結果が元ファイルと一致しません。{around}\n\n出力フォルダ:\n{output_dir}")
                self.lbl_split2_status.config(text="完了（検証NG）", foreground="gray")
                summary = "検証NG: 再構成が一致しません（詳細はエラー表示を参照）"

            plan2 = self._split2_build_plan(src_path, output_dir, full_content)
            self._split2_preview_ctx = {
                "src_path": src_path,
                "full_content": full_content,
                "plan": plan2,
            ...
            self._split2_update_preview(plan2, summary_text=summary)

            try:
                self.log(f"[2-2] 出力: {output_dir}")
            except Exception:
                pass
